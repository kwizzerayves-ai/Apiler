...
async def bot_yaniti_handler(event):
    ...
    # REPLY KONTROLÜ - msg_id ile eşleştir
    if not message.reply_to_msg_id:
        return
    
    # Hangi sorguya ait olduğunu bul
    sorgu_id = None
    ...
    try:
        ...
        if message.media and isinstance(message.media, MessageMediaDocument):
            ...
            if dosya_yolu:
                ...
                print(f"✅ Dosya alındı: {sorgu_id[:8]}")
                return
        
        # 2. BUTONLU MESAJ - KRİTİK KISIM
        if message.text and 'kayit bulundu' in message.text.lower():
            print(f"🔘 Butonlu mesaj: {sorgu_id[:8]}")
            
            if message.buttons:
                for row in message.buttons:
                    for button in row:
                        button_text = button.text
                        if 'dosya' in button_text.lower() or 'txt' in button_text.lower():
                            # 🔥 DOĞRU BUTON BASMA
                            await message.click(text=button_text)
                            sorgu_sonuclari[sorgu_id]['durum'] = 'dosya_bekleniyor'
                            print(f"📁 Butona basıldı: {button_text}")
                            return
            
            # Alternatif - ilk butona bas
            if message.buttons:
                await message.click(text=message.buttons[0][0].text)
                sorgu_sonuclari[sorgu_id]['durum'] = 'dosya_bekleniyor'
                print("📁 İlk butona basıldı")
                return
        
        # 3. DİREKT MESAJ (dosya yok, buton yok)
        if message.text and not message.media:
            sorgu_sonuclari[sorgu_id]['durum'] = 'tamam'
            sorgu_sonuclari[sorgu_id]['sonuc'] = message.text
            print(f"📝 Direkt mesaj: {sorgu_id[:8]}")
            return
...
async def bot_sorgula(komut: str, sorgu_id: str):
    # Komutu gönder ve msg_id'yi kaydet
    ...
    if sorgu_id in sorgu_sonuclari:
        del sorgu_sonuclari[sorgu_id]
    print(f"❌ Zaman aşımı: {sorgu_id[:8]}")
    return None
